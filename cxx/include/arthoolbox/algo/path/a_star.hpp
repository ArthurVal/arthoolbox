 ...
 * @tparam HeuristicScore          The result of the heuristic/distance function
 ...
 * @param[in] computeHeuristic             A function called to compute the heuristic from one node
 * @param[in] getNeighborWeightedPositions A function use to retreive valid weighted neighbors around a node
 *
...
std::deque<PositionCoordinatesType> aStarShortestPath(
  ...
  std::function<HeuristicScore(const PositionCoordinatesType &)> computeHeuristic,
  std::function<std::vector<std::pair<PositionCoordinatesType, HeuristicScore>>(const PositionCoordinatesType &)> getNeighborWeightedPositions
  ) {
